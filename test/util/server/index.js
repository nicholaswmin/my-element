...
export function createTestServer() {
  ...
  app.post('/api/user/signup', (req, res) => {
    ...
    res.status(201).json(user.createUserResponse(email))
  })
  ...
  app.post('/api/user/refresh', (req, res) => {
    ...
    res.status(201).json(user.createUserResponse('test@example.com', true))
  })

  // Generic test endpoint
  app.get('/api/test', user.validateAuth, (req, res) => {
    res.json({ data: 'success' })
  })

  // Slow operation endpoint for loading state testing
  app.get('/api/test/slow', user.validateAuth, (req, res) => {
    setTimeout(() => {
  ...
  app.post('/api/user/email/resend-verification', user.validateAuth, (req, res) => {
    // Success - empty response body
    res.status(201).send()
  })

  // Paper save endpoint - matches BAPI exactly
  app.post('/api/user/papers/save', user.validateAuth, (req, res) => {
  ...
  app.post('/api/user/papers', user.validateAuth, (req, res) => {
    res.status(201).json({
      id: 'paper-123',
  ...
  app.get('/api/user/papers', user.validateAuth, (req, res) => {
    res.json([
      { id: '1', title: 'Paper 1' },
      { id: '2', title: 'Paper 2' }
    ])
  ...
  app.get('/api/user/tags', user.validateAuth, (req, res) => {
    res.json([
      { id: '1', name: 'Important' },
      { id: '2', name: 'Personal' }
    ])
  ...
  app.post('/api/user/tags', user.validateAuth, (req, res) => {
    res.status(201).json({
      id: 'tag-' + Date.now(),
      name: req.body.name
    })
  ...
  app.post('/api/user/saved-paper/exists', user.validateAuth, (req, res) => {
    // Mock response for paper exists check
    res.json({ exists: false })
  })

  // Tags endpoints
  app.patch('/api/tags/:id', user.validateAuth, (req, res) => {
    res.json({
      id: req.params.id,
      name: req.body.name,
  ...
  app.delete('/api/tags/:id', user.validateAuth, (req, res) => {
    res.status(204).send()
  })

  // Paper endpoints
  app.get('/api/paper/:id', user.validateAuth, (req, res) => {
  ...
  app.patch('/api/paper/:id', user.validateAuth, (req, res) => {
    // BAPI returns 204 No Content for paper updates
    res.status(204).send()
  })

  app.delete('/api/paper/:id', user.validateAuth, (req, res) => {
    res.status(204).send()
  })

  app.delete('/api/papers/:id', user.validateAuth, (req, res) => {
    res.status(204).send()
  })

  // Alternative error format endpoint for testing
  app.get('/api/alt-error', user.validateAuth, (req, res) => {
    res.status(400).json({
      status: 'error',
  ...
  app.get('/api/papers/list', user.validateAuth, (req, res) => {
    res.json({
      papers: [
        {
          id: 'paper-1',
  ...
  app.get('/api/paper/check-url', user.validateAuth, (req, res) => {
    const { url } = req.query
  ...
  app.get('/api/user/preferences/editor', user.validateAuth, (req, res) => {
    res.json({
      selectedStroke: {
  ...
  app.patch('/api/user/preferences/editor', user.validateAuth, (req, res) => {
    // Return the updated preferences
    res.json(req.body)
  })

  app.get('/api/user/preferences/public', user.validateAuth, (req, res) => {
    res.json({
  ...
  app.patch('/api/user/preferences/public', user.validateAuth, (req, res) => {
    // Return the updated preferences
    res.json(req.body)
  })

  // RTC token endpoints
  app.post('/api/rtc/opentok/token', user.validateAuth, (req, res) => {
  ...
  app.get('/api/rtc/twilio/token', user.validateAuth, (req, res) => {
    res.json({
  ...
  app.post('/api/paper/:paperId/call/token', user.validateAuth, (req, res) => {
    res.json({
  ...
  app.post('/api/paper/:idSession/call/twilio', user.validateAuth, (req, res) => {
    res.json({
      token: `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCIsImN0eSI6InR3aWxpby1mcGE7dj0xIn0.${Date.now()}`,
      identity: req.user.id,
  ...
  app.post('/api/paper/:idSession/call/opentok', user.validateAuth, (req, res) => {
    res.json({
      token: `T1==cGFydG5lcl9pZD00NTcyODU5MiZzaWc9NjE5ZjRmOGQ5ZjY5NzdhNWI0${Date.now()}`,
      apiKey: '45728592',
  ...
  app.post('/api/upload/signed-url', user.validateAuth, (req, res) => {
  ...
  app.get('/api/paper/:paperId/assets/:assetKey/signed-url', user.validateAuth, (req, res) => {
    res.json({
  ...
  const serverObj = {
    ...
    createExpiredToken: user.createExpiredToken.bind(user),
    createValidToken: user.createValidToken.bind(user),
    createMalformedToken: user.createMalformedToken.bind(user)
  }
