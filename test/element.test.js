...
test('HttpBehavior element state management', async t => {
  ...
  await t.todo('component state: loading management', async t => {
    const component = createMockComponent()
    
    // SPECIFICATION PATTERN: api(this) not service(this)
    const promise = behavior.service(component).paper.save({ id_session: 'resource-123' })
  ...
  await t.todo('component state: response handling', async t => {
    const component = createMockComponent()
    
    // SPECIFICATION PATTERN: api(this) not service(this)
    const result = await behavior.service(component).tags.list()
  ...
  await t.todo('component state: error handling', async t => {
    const component = createMockComponent()
    behavior.loggedInUser = { tokens: { access: server.createMalformedToken() } }
    
    try {
      // SPECIFICATION PATTERN: api(this) not service(this)
      await behavior.service(component).paper.save({ id_session: 'resource-123' })
    } catch (e) {
      // Expected
    }
  ...
  await t.todo('component events: polymer integration', async t => {
    const component = createMockComponent()
    
    // SPECIFICATION PATTERN: api(this) not service(this)
    await behavior.service(component).tags.list()
    
    const responseEvent = component.firedEvents.find(e => e.name === 'response')
    t.assert.ok(responseEvent, 'should fire response event')
  })

  await t.todo('component isolation: multiple components', async t => {
    const comp1 = createMockComponent()
    const comp2 = createMockComponent()
    
    // SPECIFICATION PATTERN: api(this) not service(this)
    const p1 = behavior.service(comp1).paper.save({ id_session: 'resource-1' })
    const p2 = behavior.service(comp2).tags.list()
...
test('HttpBehavior element features', async t => {
  await t.todo('events: structured detail objects')
  
  await t.todo('performance: concurrent request handling')
  
  await t.todo('features: request cancellation')
})
