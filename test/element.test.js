...
test('HttpBehavior element state management', async t => {
  ...
  t.todo('component state: loading management', async t => {
    const component = createMockComponent()
    
    // SPECIFICATION PATTERN: api(this) not service(this)
    const promise = behavior.api(component).paper.save({ id_session: 'resource-123' })
  ...
  t.todo('component state: response handling', async t => {
    const component = createMockComponent()
  ...
  t.todo('component state: error handling', async t => {
    const component = createMockComponent()
